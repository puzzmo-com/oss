import type { Game, MutationResolvers, PredictionResolvers, Prediction as PredictionType, QueryResolvers, User } from "types/graphql"

import { getFirstUserFromContext } from "src/lib/auth"
import { db } from "src/lib/db"

type PartialGame = Omit<Game, "homeTeam" | "awayTeam" | "predictions" | "season">
...
const getPredictionStatus = (prediction: PartialPrediction): "incomplete" | "correct" | "incorrect" => {
...
export const standings: QueryResolvers["standings"] = async ({ seasonId }) => {
  ...
  const userIdRankings = Object.entries(userPredictionMap).map(([userId, predictions]) => {
...
export const myPredictions: QueryResolvers["myPredictions"] = (_temp, { context }) => {
...
export const createPrediction: MutationResolvers["createPrediction"] = ({ input }) => {
  return db.prediction.create({
    data: input,
  })
}

export const updatePrediction: MutationResolvers["updatePrediction"] = ({ id, input }) => {
...
export const deletePrediction: MutationResolvers["deletePrediction"] = ({ id }) => {
...
export const Prediction: PredictionResolvers = {
  ...
  team: (_obj, { root }) => db.prediction.findUnique({ where: { id: root.id } }).team(),
  game: (_obj, { root }) => db.prediction.findUnique({ where: { id: root.id } }).game(),
}
