import type { MutationResolvers, QueryResolvers, TeamResolvers } from "types/graphql"
...
export const Team: TeamResolvers = {
  ...
  Prediction: (_obj, { root }) => db.team.findUnique({ where: { id: root.id } }).Prediction(),
  homeTeamGames: (_obj, { root }) => db.team.findUnique({ where: { id: root.id } }).homeTeamGames(),
  awayTeamGames: (_obj, { root }) => db.team.findUnique({ where: { id: root.id } }).awayTeamGames(),
}
