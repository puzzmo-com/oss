...
export const createSharedSchemaFiles = (context: AppContext, verbose: boolean) => {
  const step = makeStep(verbose)
  step("Creating shared schema files", () => createSharedExternalSchemaFile(context))
  step("Creating shared return position schema files", () => createSharedReturnPositionSchemaFile(context))
